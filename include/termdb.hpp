...
namespace tdb {
	...
	void TermDb::_emulate_cup(const short jump = 1) const noexcept
	{
		if (jump > 0) {
			...
			short destRow = buffInfo.srWindow.Top >= row ? buffInfo.srWindow.Top : row;
			SetConsoleCursorPosition(consoleHandle,
	...
	void TermDb::_emulate_cud(const short jump = 1) const noexcept
	{
		if (jump > 0) {
			...
			auto row = buffInfo.dwCursorPosition.Y + jump;
			short destRow = buffInfo.srWindow.Bottom <= row ? buffInfo.srWindow.Bottom : row;
			SetConsoleCursorPosition(consoleHandle,
				{ buffInfo.dwCursorPosition.X, destRow });
		}
	}
	...
	void TermDb::_emulate_cuf(const short jump = 1) const noexcept
	{
		if (jump > 0) {
			...
			short destCol = buffInfo.srWindow.Right <= col ? buffInfo.srWindow.Right : col;
			SetConsoleCursorPosition(consoleHandle,
	...
	void TermDb::_emulate_cub(const short jump = 1) const noexcept
	{
		if (jump > 0) {
			...
			auto col = buffInfo.dwCursorPosition.X - jump;
			short destCol = buffInfo.srWindow.Left >= col ? buffInfo.srWindow.Left : col;
			SetConsoleCursorPosition(consoleHandle,
				{ destCol, buffInfo.dwCursorPosition.Y });
		}
	}
